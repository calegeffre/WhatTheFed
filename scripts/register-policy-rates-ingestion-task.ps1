param(
    [string]$TaskName = "WhatTheFed Policy Rates Ingestion",
    [string]$RunAt = "07:00",
...
$runnerScript = Join-Path $repoRoot "scripts\run-policy-rates-ingestion.ps1"
...
# The NY Fed posts reference rates around 08:00 ET on business days.
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -TaskPath $TaskPath `
    -Description "Weekday ingestion of NY Fed overnight reference rates (EFFR, SOFR, OBFR, BGCR, TGCR) for WhatTheFed." `
    -Action $taskAction `
    -Trigger $taskTrigger `
